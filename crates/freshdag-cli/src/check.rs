...
pub fn run(store_root: &Path, artifact: &str) -> Result<Checked, CheckError> {
    ...
    // No special case for a missing recipe hash any more: the engine
    // now caps such a computation at `unknown` with a
    // `recipe-identity-unavailable` reason, so it arrives as an ordinary
    // certificate that explains itself rather than as an engine refusal
    // the CLI had to reinterpret.
